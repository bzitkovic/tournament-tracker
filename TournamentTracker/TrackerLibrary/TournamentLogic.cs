    ...
    public static class TournamentLogic
    {
        ...
        private static void AlertPersonToNewRound(PersonModel p, string teamName, MatchupEntryModel competitor)
        {
            ...
            if (competitor != null)
            {
                ...
                body.AppendLine();
                body.AppendLine();
                body.AppendLine("Enjoy the tournament!");
                body.AppendLine("~Admin Team");
            }
            else
            {
                subject = $"You have a free pass this round!";

                body.AppendLine("You have a round off!");
                body.AppendLine("~Admin Team");
            }
        ...
        private static void CompleteTournament(TournamentModel model)
        {
            ...
            subject = $"Winner of {model.TournamentName} is: {winners.TeamName }!";

            body.AppendLine("<h1> WINNER! </h1>");
            body.Append("<p> Congrats! </p>");
            body.AppendLine("<br />");

            if (winnerPrize > 0)
            {
                body.AppendLine($"<p> { winners.TeamName } won €{ winnerPrize } </p>");
            }

            if (runnerUpPrize > 0)
            {
                body.AppendLine($"<p> { runnerUp.TeamName } won €{ runnerUpPrize } </p>");
            }
            ...
            body.AppendLine("<p> See you again! :) </p>");
            body.AppendLine("~Admin Team");
        ...
        private static void MarkWinnersInMatchups(List<MatchupModel> models)
        {
            ...
            foreach (MatchupModel m in models)
            {
                ...
                if (greaterWins == "0")
                {
                    ...
                    {
                        throw new Exception("Ties aren't alowed!");
                    }
                }
                else
                {
        ...
        private static void CreateOtherRounds(TournamentModel model, int numberOfRouds)
        {
            int round = 2;
